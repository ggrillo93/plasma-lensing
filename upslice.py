...
def planeSlice(uxmax, dso, dsl, f, dm, m, n, ax, ay, npoints = 100):
    ...
    def realGain(roots, gains):
        ...
        def abcoeff(f1, f2, U1, U2):
            ...
            air = airy(xi1)
            # print(air)
            a = (-xi1)**0.25 * (U1 + 1j*U2) * air[0]
            b = -1j*(-xi1)**-0.25 * (U1 - 1j*U2) * air[1]
            return np.array([a, b])
        ...
        # # # print([a, b])
        # I1 = pi**0.5*exp(1j*(lc*chi(phi12, phi22) - 0.25*pi))*(a + b)
        # G1 = np.abs((I1 + U32)**2)
        # # # Second caustic
        # c, d = abcoeff(phi31, phi21, U31, U21)
        # # # print([c, d])
        # I2 = pi**0.5*exp(1j*(lc*chi(phi31, phi21) - 0.25*pi))*(c + d)
        # G2 = np.abs((I2 + U11)**2)
    ...
    if ncross == 2: # 2 dark regions with 1 image, 1 bright region with 3 images
        ...
        d1upx = np.linspace(-uxmax, upcross[0][0] - 1e-3, npoints/2) # d = dark, b = bright
        bupx = np.linspace(upcross[0][0] + 1e-3, upcross[1][0] - 1e-3, npoints*2)
        d2upx = np.linspace(upcross[1][0] + 1e-3, uxmax, npoints/2)
...
def planeSliceG(uxmax, uymax, dso, dsl, f, dm, m, n, ax, ay, npoints = 100, gsizex = 2048, gsizey = 2048):
    ...
    if ncross == 2: # 2 dark regions with 1 image, 1 bright region with 3 images
        # Create slice by segments
        d1upx = np.linspace(xmin, upcross[0][0] - 1e-3, npoints/2) # d = dark, b = bright
        bupx = np.linspace(upcross[0][0] + 1e-3, upcross[1][0] - 1e-3, npoints*2)
        d2upx = np.linspace(upcross[1][0] + 1e-3, xmax, npoints/2)
        upxvec = np.concatenate((d1upx, bupx, d2upx))
        segs = np.array([lineVert(d1upx, m, n), lineVert(bupx, m, n), lineVert(d2upx, m, n)])
        nsolns = np.array([1, 3, 1]) # Number of expected solutions at each segment
        ...
        if dm > 0: # Positive DM. 3 dark regions with 1 image, 2 bright regions with 3 images.
            # Create slice by segments
            d1upx = np.linspace(xmin, upcross[0][0] - 1e-3, npoints/2) # d = dark, b = bright
            b1upx = np.linspace(upcross[0][0] + 1e-3, upcross[1][0] - 1e-3, npoints*2)
            d2upx = np.linspace(upcross[1][0] + 1e-3, upcross[2][0] - 1e-3, npoints/2)
            b2upx = np.linspace(upcross[2][0] + 1e-3, upcross[3][0] - 1e-3, npoints*2)
            d3upx = np.linspace(upcross[3][0] + 1e-3, xmax, npoints/2)
            upxvec = np.concatenate((d1upx, b1upx, d2upx, b2upx, d3upx))
            segs = np.array([lineVert(d1upx, m, n), lineVert(b1upx, m, n), lineVert(d2upx, m, n), lineVert(b2upx, m, n), lineVert(d3upx, m, n)])
            nsolns = np.array([1, 3, 1, 3, 1])
        ...
        if dm < 0: # Negative DM. 2 dark regions with 1 image, 2 bright regions with 3 images, 1 bright region with 5 images.
            d1upx = np.linspace(xmin, upcross[0][0] - 1e-3, npoints/2) # d = dark, b = bright
            b1upx = np.linspace(upcross[0][0] + 1e-3, upcross[1][0] - 1e-3, 2*npoints)
            b2upx = np.linspace(upcross[1][0] + 1e-3, upcross[2][0] - 1e-3, 2*npoints)
            b3upx = np.linspace(upcross[2][0] + 1e-3, upcross[3][0] - 1e-3, 2*npoints)
            d2upx = np.linspace(upcross[3][0] + 1e-3, xmax, npoints/2)
            upxvec = np.concatenate((d1upx, b1upx, b2upx, b3upx, d2upx))
            segs = np.asarray([lineVert(upx, m, n) for upx in upxvec])
            nsolns = np.array([1, 3, 5, 3, 1])
    ...
    # Calculate gains
    G1 = np.array([]) # GO gains
    for i in range(len(nsolns)):
        fields = allfields[i][0]
        if nsolns[i] == 1:
            G = np.zeros(len(fields))
            for j in range(len(fields)):
                G[j] = np.abs(fields[j])**2
        else:
    ...
    #     fields = allfields[i]
    #     if nsolns[i] == 1:
    #         G = np.zeros(len(fields[0]))
    #         for j in range(len(fields[0])):
    #             G[j] = np.abs(fields[1][j])**2
    #     else:
    #         G = np.zeros(len(fields[1][0]))
    #         for j in range(len(fields[1][0])):
    #             G[j] = np.abs(fields[1][2][j])**2
    #     G2 = np.append(G, G2)

    pgains = []
    for u in ucross:
        gain = np.abs(physField(u, rF2, lc, ax, ay))**2
        pgains.append(gain)
    ...
    ax1.plot(upxvec, G1, color = 'red') # Plot GO gain
    # ax1.plot(upxvec, G2, color = 'green')
    ax1.scatter(upcross.T[0], pgains, color = 'black')
